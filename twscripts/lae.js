...
if(document.location.href.endsWith("am_farm")) {
    if(classExists) {
        window.location.reload();
    }
    else {
        var version = "1.13.1 - Modified by Brannstrom";
        var scriptName = "LA Enhancer (1.13.1) - Modified by Brannstrom";
    ...
    if (cansend && filtersApplied) {
        if (!checkIfNextVillage()) {
            ...
            if (button.hasClass("farm_icon_disabled") || button.html() == undefined) {
                ...
                window.top.UI.ErrorMessage("Hmmm... her skjedde det noe. Ny by :)", 500);
				getNewVilage("n");
            }
            else {
                button.click();
...
function uglyHider(linker) {
    ...
    document.getElementById("manager_icon_farm").click();
}
void(0);